...
#pragma omp parallel for schedule(dynamic)  // start parallel region
  for (int i = 0; i < biggest_possible_number; ++i) {
    if (final_solution < i)  // our possible solution is smaller than the solution that was already found
      continue;
    if (is_solution(i)) {     // find some solution, not necessary the smallest
      int current = final_solution.load();	// get current final solution 
      while (current > i && !final_solution.compare_exchange_weak(current, i)) {}
    }
  }  // end parallel region
